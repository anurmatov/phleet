...
| Any | Claude + local model (Anthropic-compatible server, e.g. Ollama) | ⚠️ A per-agent `AnthropicBaseUrl` runs Claude Code against a local server with no Claude credential in the container. Exact-head canary pending. See `docs/providers/claude-local-models.md` |
