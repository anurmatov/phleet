...
/// <summary>
/// The <c>/api/project-contexts</c> surface: list, detail, create, new version, rollback and
/// toggle-active.
/// </summary>
/// <remarks>
/// Lifted out of <c>Program.cs</c> (#347) — the output-style precedent — so the endpoint tests map
/// the SAME handlers a live orchestrator maps. #346 adds the <c>size</c> report
/// (<see cref="PromptSizePolicy"/>) to the three writes, after the commit.
...
public static class ProjectContextEndpoints
{
    ...
    public static IEndpointRouteBuilder MapProjectContextEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapPost("/api/project-contexts/{name}/versions", async (string name, HttpRequest request, IServiceScopeFactory scopeFactory, [FromServices] PromptSizePolicy sizePolicy) =>
        {
            ...
            return Results.Ok(new { message = $"Project context '{name}' updated to v{newVersion}", version = newVersion, size });
        });

        // REST: rollback project context to a prior version (creates new version with old content)
        app.MapPost("/api/project-contexts/{name}/rollback/{targetVersion:int}", async (string name, int targetVersion, IServiceScopeFactory scopeFactory, [FromServices] PromptSizePolicy sizePolicy) =>
        {
            using var scope = scopeFactory.CreateScope();
            var db = scope.ServiceProvider.GetService<OrchestratorDbContext>();
            if (db is null)
